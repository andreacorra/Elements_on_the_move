...
#extract and finalize Year 1 data
  ...
  # Modern sf replacement for SpatialMultiPointsDataFrame
  spmdf_Yr1 <- cbind(Year_1_New.df,
                     Longitude.deg. = Year_1_New$Longitude.deg.,
                     Latitude.deg. = Year_1_New$Latitude.deg.,
                     Split_Var = Year_1_New$DateTime2_hour) %>%
    st_as_sf(coords = c("Longitude.deg.", "Latitude.deg."), crs = 4326, na.fail = FALSE) %>%
    group_by(Split_Var) %>%
    summarize(
      Year = first(Year), Month = first(Month), Day = first(Day),
      Hour = first(Hour), DateTime = first(DateTime), New_AID = first(New_AID),
      count = n(),
      geometry = st_cast(st_combine(geometry), "MULTIPOINT"),
      .groups = "drop"
    ) %>%
    select(-Split_Var)
 ...
 Add_Year_3_Start$MonthDayYear<-format(Add_Year_3_Start$DateTime2_hour, format="%y-%m-%d")
  ...
  # Modern sf replacement for SpatialMultiPointsDataFrame
  spmdf_Yr2 <- cbind(Year_2_New.df,
                     Longitude.deg. = Year_2_New$Longitude.deg.,
                     Latitude.deg. = Year_2_New$Latitude.deg.,
                     Split_Var = Year_2_New$DateTime2_hour) %>%
    st_as_sf(coords = c("Longitude.deg.", "Latitude.deg."), crs = 4326, na.fail = FALSE) %>%
    group_by(Split_Var) %>%
    summarize(
      Year = first(Year), Month = first(Month), Day = first(Day),
...
#Getting Backtrack data
  ...
  spmdf_Yr1 <- cbind(Year_1_New.df,
                     Longitude.deg. = Year_1_New$Longitude.deg.,
                     Latitude.deg. = Year_1_New$Latitude.deg.,
                     Split_Var = Year_1_New$New_Datetime) %>%
    st_as_sf(coords = c("Longitude.deg.", "Latitude.deg."), crs = 4326, na.fail = FALSE) %>%
    group_by(Split_Var) %>%
    summarize(
      Year = first(Year), Month = first(Month), Day = first(Day),
